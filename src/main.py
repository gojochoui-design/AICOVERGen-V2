...
from mdx import run_mdx, validate_onnx, redownload_mdx_model
...
def ensure_mdx_models_ready():
    """Check that all required MDX models exist and are not corrupted.
    If a model is missing or corrupted, attempt to download/re-download it.
    
    Returns:
        list: Names of models that could not be prepared (empty = all OK)
    """
    required_models = [
    ...
    for model_name in required_models:
        ...
        if not os.path.exists(model_path):
            # Model is missing, try to download it
            print(f"[PRE-FLIGHT] MDX model missing: {model_name}. Downloading...")
            try:
                from src.download_models import dl_model, MDX_DOWNLOAD_LINK
                from pathlib import Path
                dl_model(MDX_DOWNLOAD_LINK, model_name, Path(mdxnet_models_dir))
            except Exception as e:
                print(f"[PRE-FLIGHT] Failed to download {model_name}: {e}")
        ...
        if not is_valid:
            print(f"[PRE-FLIGHT] MDX model corrupted: {model_name} ({reason}). Re-downloading...")
            if not redownload_mdx_model(model_name, mdxnet_models_dir):
                failed.append(model_name)
        else:
            size_mb = os.path.getsize(model_path) / (1024 * 1024)
            print(f"[PRE-FLIGHT] MDX model OK: {model_name} ({size_mb:.1f}MB)")
...
def preprocess_song(song_input, mdx_model_params, song_id, is_webui, input_type, progress, keep_orig, orig_song_path):
    ...
    # Pre-flight check: ensure MDX models are ready before processing
    failed_models = ensure_mdx_models_ready()
    if failed_models:
        error_msg = f'MDX models are missing or corrupted and could not be downloaded: {", ".join(failed_models)}. Check your internet connection and try running: python src/download_models.py'
        raise_exception(error_msg, is_webui)
...
def song_cover_pipeline(song_input, voice_model, pitch_change, keep_files,
    ...
    try:
        ...
        try:
            ...
            mdx_model_params = {"UVR-MDX-NET-Voc_FT.onnx": {"mdx_dim_f_set": 3072, "mdx_dim_t_set": 8, "mdx_n_fft_scale_set": 6144, "mdx_hop_set": 1024}, "UVR-MDX-NET-Inst_HQ_4.onnx": {"mdx_dim_f_set": 2048, "mdx_dim_t_set": 8, "mdx_n_fft_scale_set": 4096, "mdx_hop_set": 1024}}
