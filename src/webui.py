...
# Retrocompatibilidad: cualquier referencia externa a `mdxnet_models_dir`
# sigue funcionando apuntando a la nueva carpeta `separator_models`.
...
# ============================================================
# PITCH MODE SYSTEM - Cascading dropdown
# ============================================================

PITCH_MODES = [
    "Original (Semitonos)",
    "Octava + Semitono (Personalizable)",
    "Conversion de Genero (Hombre <-> Mujer)",
]

def update_pitch_controls(pitch_mode):
    """Toggle visibility of pitch sub-controls based on selected mode."""
    if pitch_mode == PITCH_MODES[0]:
        # Original mode: show simple slider, hide octave/fine and gender
        return (
            gr.update(visible=True),   # pitch_original
            gr.update(visible=False),  # pitch_octave
            gr.update(visible=False),  # pitch_fine
            gr.update(visible=False),  # pitch_gender
        )
    elif pitch_mode == PITCH_MODES[1]:
        # Octava + Semitono mode: hide original, show octave and fine, hide gender
        return (
            gr.update(visible=False),  # pitch_original
            gr.update(visible=True),   # pitch_octave
            gr.update(visible=True),   # pitch_fine
            gr.update(visible=False),  # pitch_gender
        )
    elif pitch_mode == PITCH_MODES[2]:
        # Gender conversion mode: hide original, hide octave/fine, show gender
        return (
            gr.update(visible=False),  # pitch_original
            gr.update(visible=False),  # pitch_octave
            gr.update(visible=False),  # pitch_fine
            gr.update(visible=True),   # pitch_gender
        )
    else:
        return (
...
def update_gender_label(gender_val):
    """Update the gender slider label based on value for clarity."""
    if gender_val > 0:
        direction = "Hombre -> Mujer"
    elif gender_val < 0:
        direction = "Mujer -> Hombre"
    else:
        direction = "Sin cambio"
    return gr.update(label=f'Pitch Genero ({direction})  |  {gender_val * 3:+d} semitonos')


def compute_final_pitch(pitch_mode, pitch_original, pitch_octave, pitch_fine, pitch_gender):
    """Compute the final pitch change value based on the selected mode and sub-controls.

    - Original mode: pitch_original is the value in semitones (-24 to 24)
    - Octava + Semitono mode: (pitch_octave * 12) + pitch_fine
      pitch_octave: -2 to +2 (each = 12 semitones)
      pitch_fine: -11 to +11 (fine-tuning within the octave)
    - Gender conversion mode: pitch_gender * 3
      pitch_gender: -4 to +4 (each step = 3 semitones)
      +4 = +12 semitones (1 octave up, male -> female)
      -4 = -12 semitones (1 octave down, female -> male)
    """
    if pitch_mode == PITCH_MODES[0]:
        return int(pitch_original)
    elif pitch_mode == PITCH_MODES[1]:
        return int(pitch_octave) * 12 + int(pitch_fine)
    elif pitch_mode == PITCH_MODES[2]:
        return int(pitch_gender) * 3
    else:
        return int(pitch_original)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--theme', type=str, default="soft", help='Set the theme (default: soft)')
    ...
    with gr.Blocks(title='AICOVERGEN Remaster', theme=args.theme, fill_width=True, fill_height=False) as app:
        gr.Label(f'AICOVERGEN Remaster', show_label=False)
        if IS_ZERO_GPU:
            gr.Markdown(
                """
                <details>
        ...
        with gr.Tab("Generate"):
            ...
            with gr.Accordion('Main Options'):
                with gr.Row():
                    ...
                    with gr.Column():
                        # ============================================================
                        # NEW PITCH MODE SYSTEM
                        # ============================================================
                        pitch_mode = gr.Dropdown(
                            PITCH_MODES,
                            value=PITCH_MODES[0],
                            label='Modo de Pitch',
                            info='Selecciona como quieres controlar el cambio de pitch. Cambia esta opcion y los controles se recargaran automaticamente.'
                        )

                        # --- Original mode slider ---
                        pitch_original = gr.Slider(
                            -24, 24, value=0, step=1,
                            label='Pitch Change (Vocals ONLY)',
                            info='Cambio de pitch en semitonos. 12 semitonos = 1 octava.',
                            visible=True
                        )

                        # --- Octava + Semitono mode sliders ---
                        with gr.Group(visible=False) as pitch_octave_group:
                            pitch_octave = gr.Slider(
                                -2, 2, value=0, step=1,
                                label='Octavas (1 = 12 semitonos)',
                                info='Cada paso = 1 octava (12 semitonos). -2 = dos octavas abajo, +2 = dos octavas arriba.'
                            )
                            pitch_fine = gr.Slider(
                                -11, 11, value=0, step=1,
                                label='Ajuste Fino (Semitonos)',
                                info='Ajuste preciso dentro de la octava. -11 a +11 semitonos para afinar el pitch exacto.'
                            )
                            pitch_octave_preview = gr.Textbox(
                                value='Total: 0 semitonos',
                                label='Preview Pitch Total',
                                interactive=False
                            )

                            def update_octave_preview(octave_val, fine_val):
                                total = int(octave_val) * 12 + int(fine_val)
                                sign = '+' if total >= 0 else ''
                                oct_sign = '+' if int(octave_val) >= 0 else ''
                                return f'{oct_sign}{int(octave_val)} octavas + {sign}{int(fine_val)} semitonos = {sign}{total} semitonos'

                            pitch_octave.change(update_octave_preview, [pitch_octave, pitch_fine], pitch_octave_preview)
                            pitch_fine.change(update_octave_preview, [pitch_octave, pitch_fine], pitch_octave_preview)

                        # --- Gender conversion mode slider ---
                        pitch_gender = gr.Slider(
                            -4, 4, value=0, step=1,
                            label='Pitch Genero (Sin cambio)  |  +0 semitonos',
                            info='Cada paso = 3 semitonos. +4 = +12 semitonos (Hombre->Mujer). -4 = -12 semitonos (Mujer->Hombre).',
                            visible=False
                        )

                        # Connect mode change to visibility toggles
                        pitch_mode.change(
                            update_pitch_controls,
                            inputs=[pitch_mode],
                            outputs=[pitch_original, pitch_octave, pitch_fine, pitch_gender]
                        )
                        # Also toggle the octave group visibility
                        pitch_mode.change(
                            lambda mode: gr.update(visible=(mode == PITCH_MODES[1])),
                            inputs=[pitch_mode],
                            outputs=[pitch_octave_group]
                        )
                        # Update gender label dynamically
                        pitch_gender.change(
                            update_gender_label,
                            inputs=[pitch_gender],
                            outputs=[pitch_gender]
                        )

                        # Overall pitch change (applies to everything together)
            ...
            with gr.Accordion('Voice conversion options', open=False):
                with gr.Row():
                    ...
                    with gr.Column():
                        f0_method = gr.Dropdown(['rmvpe', 'fcpe', 'crepe', 'crepe-tiny', 'hybrid[rmvpe+fcpe]'], value='rmvpe', label='Pitch detection algorithm', info='Best option is rmvpe (clarity in vocals), then mangio-crepe (smoother vocals), rmvpe+ use a minimum and maximum allowed pitch values.')
            ...
            def wrapped_song_cover_pipeline(*args):
                # args: [local_file, rvc_model, pitch_mode, pitch_original, pitch_octave, pitch_fine, pitch_gender,
                #        keep_files, is_webui, main_gain, backup_gain, inst_gain, index_rate, filter_radius,
                #        rms_mix_rate, f0_method, crepe_hop_length, protect, pitch_all, reverb_rm_size,
                #        reverb_wet, reverb_dry, reverb_damping, output_format, extra_denoise, steps]
                args_list = list(args)

                # Extract pitch mode and sub-values
                pitch_mode_val = args_list[2]
                pitch_original_val = args_list[3]
                pitch_octave_val = args_list[4]
                pitch_fine_val = args_list[5]
                pitch_gender_val = args_list[6]

                # Compute final pitch from mode
                final_pitch = compute_final_pitch(
                    pitch_mode_val,
                    pitch_original_val,
                    pitch_octave_val,
                    pitch_fine_val,
                ...
                # Replace args: remove pitch_mode, pitch_original, pitch_octave, pitch_fine, pitch_gender
                # and insert final_pitch as the single pitch_change value
                # Original pipeline expects: [local_file, rvc_model, pitch_change, keep_files, ...]
                new_args = [args_list[0], args_list[1], final_pitch]  # local_file, rvc_model, pitch_change
                new_args.extend(args_list[7:])  # keep_files onwards

                return song_cover_pipeline(*new_args)

            generate_btn.click(wrapped_song_cover_pipeline,
                               inputs=[local_file, rvc_model, pitch_mode, pitch_original, pitch_octave, pitch_fine, pitch_gender,
                                       keep_files, is_webui, main_gain, backup_gain,
            ...
            clear_btn.click(lambda: [0, 0, 0, 0, 0.5, 3, 0.25, 0.33, 'rmvpe', 128, 0, 0.15, 0.2, 0.8, 0.7, 'mp3', None, True, 1],
                            outputs=[pitch_original, main_gain, backup_gain, inst_gain, index_rate, filter_radius, rms_mix_rate,
