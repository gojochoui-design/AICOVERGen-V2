...
class CommonSeparator:
    ...
    def write_audio_soundfile(self, stem_path: str, stem_source):
        ...
        #
        # Parche AICoverGen: cuando el input es un formato comprimido (MP3, OGG,
        # FLAC, etc.) su subtype (p.ej. MPEG_LAYER_III) NO es válido como subtype
        # de salida para un WAV. El código original usaba ciegamente el subtype
        # del input, lo que rompía al escribir stems WAV a partir de un MP3.
        # La solución es: si el subtype del input NO empieza por 'PCM_' y el
        # formato de salida es WAV, caer al mapeo por bit depth (que siempre da
        # un subtype PCM válido).
        output_subtype = None
        input_is_compressed = (
            self.input_subtype is not None
            and not self.input_subtype.startswith("PCM_")
            and not self.input_subtype.startswith("FLOAT")
            and not self.input_subtype.startswith("DOUBLE")
        )

        if self.input_subtype and not input_is_compressed:
            ...
            if input_is_compressed:
                self.logger.info(
                    f"Input subtype {self.input_subtype!r} is compressed; "
                    f"using PCM subtype for WAV output: {output_subtype}"
                )
            else:
