...
try:
    ...
    # onnxruntime es opcional para los modelos Mel-RoFormer / BS-RoFormer
    # (que usan torch directamente). Sólo se necesita para MDX/VR/MDXC-onnx.
    # Si no está instalado, definimos un stub mínimo para que la detección
    # de providers no rompa.
    ort = None
...
class Separator:
    ...
    def __init__(
        ...
        if not info_only:
            package_dist = self.get_package_distribution("audio-separator")
            package_version = package_dist.version if package_dist is not None else "0.44.5-vendored"
    ...
    def setup_torch_device(self, system_info):
        ...
        # onnxruntime es opcional: si no está instalado, asumimos lista vacía
        # de providers (sólo afecta a la ruta MDX/VR, no a RoFormer).
        ort_providers = ort.get_available_providers() if ort is not None else []
